// A>V>C> avc.programming@gmail.com https://sites.google.com/site/avcplugins/
// Ignore Spelling: Ver

namespace AVC
{
  public static class
  FileL
  {
    public static readonly string[] File = {
      "File",
      "Файл",
      "File",
      "Datei",
      "文件",
      "Archivo",
      "Fichier",
      "Dosya"};

    public static readonly string[] Folder = {
      "Folder",
      "Папка",
      "Cartella",
      "Mappe",
      "文件夹",
      "Carpeta",
      "Dossier",
      "Klasör"};

    public static readonly string[] MkDir = {
      "Creating the directory '{0}'",
      "Создается каталог '{0}'",
      "Creazione di una directory '{0}'",
      "Verzeichnis wird erstellt '{0}'",
      "创建目录'{0}'",
      "Creando el directorio '{0}'",
      "Création du répertoire '{0}'",
      "Dizin oluşturuluyor '{0}'"};

    public static string[] FileDialogTitle = {
      "Open file",
      "Открыть файла",
      "Apri il file",
      "Datei öffnen",
      "打开文件",
      "Abrir archivo",
      "Ouvrir le fichier",
      "Dosyayı aç"};

    public static readonly string[] PathNotExists = {
      "The path does not exist or is not available '{0}'",
      "Не существует или недоступен путь '{0}'",
      "Il percorso non esiste o non è disponibile '{0}'",
      "Verzeichnis wird erstellt '{0}'",
      "路径不存在或不可用'{0}'",
      "La ruta no existe o no está disponible '{0}'",
      "Le chemin n'existe pas ou n'est pas disponible '{0}'",
      "Yol mevcut değil veya kullanılamıyor '{0}'"};

    public static readonly string[] NoFile = {
      "File not specified",
      "Не задан файл",
      "File non specificato.",
      "Datei nicht angegeben.",
      "未指定文件",
      "Archivo no especificado",
      "Fichier non spécifié",
      "Dosya belirtilmedi"};

    public static readonly string[] ExtensionError = {
      "Invalid file type '{0}'",
      "Недопустимый тип файла '{0}'",
      "Tipo di file '{0}' non valido",
      "Ungültiger Dateityp '{0}'",
      "檔案類型「{0}」無效",
      "Tipo de archivo '{0}' no válido",
      "Type de fichier '{0}' invalide",
      "Geçersiz dosya türü '{0}'"};

    public static readonly string[] FileAccessErr = {
      "Someone has already opened the file and now it's impossible to read it: {0}",
      "Кто-то уже открыл файл и теперь его невозможно прочитать: {0}",
      "Qualcuno ha già aperto il file e ora è impossibile leggerlo: {0}",
      "Jemand hat die Datei bereits geöffnet und jetzt ist es unmöglich, sie zu lesen: {0}",
      "有人已经打开了文件，现在无法阅读：{0}",
      "Alguien ya ha abierto el archivo y ahora es imposible leerlo: {0}",
      "Quelqu'un a déjà ouvert le fichier et maintenant il est impossible de le lire: {0}",
      "Birisi dosyayı zaten açtı ve şimdi okumak imkansız: {0}"};

    public static readonly string[] FileNotExist = {
      "File '{0}' does not exist",
      "Не существует файл '{0}'",
      "Il file '{0}' non esiste.",
      "Datei '{0}' existiert nicht.",
      "文件'{0}'不存在",
      "El archivo '{0}' no existe",
      "Le fichier '{0}' n'existe pas",
      "Dosya '{0}' mevcut değil"};

    public static readonly string[] FailedReadFile = {
      "Failed to read file '{0}'\n\t{1}",
      "Не удалось прочитать файл '{0}'.\n\t{1}",
      "Impossibile leggere il file '{0}'.\n\t{1}",
      "Fehler beim Lesen der Datei '{0}'.\n\t{1}",
      "未能读取文件'{0}'\n\t{1}",
      "No se pudo leer el archivo '{0}'\n\t{1}",
      "Impossible de lire le fichier '{0}'\n\t{1}",
      "Dosya '{0}' okunamadı\n\t{1}"};

    public static readonly string[] FailedWriteFile = {
      "Failed to write file '{0}'.\n\t{1}",
      "Не удалось записать файл '{0}'.\n\t{1}",
      "Impossibile scrivere il file '{0}'.\n\t{1}",
      "Fehler beim Schreiben der Datei '{0}'.\n\t{1}",
      "无法写入文件'{0}'\n\t{1}",
      "No se pudo escribir el archivo '{0}'.\n\t{1}",
      "Impossible d'écrire le fichier '{0}'.\n\t{1}",
      "Dosya '{0}' yazılamadı.\n\t{1}"};

    public static readonly string[] CopyError = {
      "Failed to copy file '{0}'.\n\t{1}",
      "Не удалось скопировать файл '{0}'.\n\t{1}",
      "Impossibile copiare il file '{0}'.\n\t{1}",
      "Fehler beim Kopieren der Datei '{0}'.\n\t{1}",
      "无法复制文件'{0}'。\n\t{1}",
      "No se pudo copiar el archivo '{0}'.\n\t{1}",
      "Impossible de copier le fichier '{0}'.\n\t{1}",
      "Dosya '{0}' kopyalanamadı.\n\t{1}"};

    public static readonly string[] NotRequireUpdating = {
      "Doesn't require updating file '{0}'.",
      "Не требует обновлять файл '{0}'.",
      "Non richiede l'aggiornamento del file '{0}'.",
      "Die Datei '{0}' muss nicht aktualisiert werden.",
      "不需要更新文件'{0}'。",
      "No requiere actualizar el archivo '{0}'.",
      "Ne nécessite pas la mise à jour du fichier '{0}'.",
      "Dosya '{0}' güncellenmesini gerektirmiyor."};

    public static readonly string[] CopyingFile = {
      "Copying file '{0}' to folder '{1}'.",
      "Копирование файла '{0}' в папку '{1}'.",
      "Copia del file '{0}' nella cartella '{1}'.",
      "Kopieren der Datei '{0}' in den Ordner '{1}'.",
      "正在将文件'{0}'复制到文件夹'{1}'。",
      "Copiando archivo '{0}' a la carpeta '{1}'.",
      "Copie du fichier '{0}' dans le dossier '{1}'.",
      "Dosya '{0}' klasöre '{1}' kopyalanıyor."};

    public static readonly string[] ReadingFile = {
      "  Reading file '{0}'",
      "  Чтение файла '{0}'",
      "  Lettura del file '{0}'",
      "  Datei '{0}' wird gelesen",
      "  正在读取文件'{0}'",
      "  Leyendo archivo '{0}'",
      "  Lecture du fichier '{0}'",
      "  Dosya '{0}' okunuyor"};

    public static readonly string[] SavingFile = {
      "  Saving file '{0}'",
      "  Сохранение файла '{0}'",
      "  Salvataggio del file '{0}'",
      "  Datei '{0}' wird gespeichert",
      "  正在保存文件 {0}。",
      "  Guardando archivo '{0}'",
      "  Enregistrement du fichier '{0}'",
      "  Dosya '{0}' kaydediliyor"};

    public static readonly string[] SavingFiles = {
      "Saving files",
      "Сохранение файлов",
      "Salvataggio di file",
      "Dateien speichern",
      "保存文件",
      "Guardando archivos",
      "Enregistrement de fichiers",
      "Dosyalar kaydediliyor"};

    public static readonly string[] DeletingFiles = {
      "  Deleting file '{0}'",
      "  Удаление файла '{0}'",
      "  Eliminazione del file '{0}'",
      "  Datei '{0}' wird gelöscht",
      "  正在删除文件'{0}'",
      "  Eliminando archivo '{0}'",
      "  Suppression du fichier '{0}'",
      "  Dosya '{0}' siliniyor"};

    public static readonly string[] FileCopied = {
      "Successfully copied '{0}' \tto '{1}'",
      "Скопирован файл '{0}' \tв папку '{1}'",
      "Copiato con successo '{0}' \t-> '{1}'",
      "Erfolgreich kopiert '{0}' \t-> '{1}'",
      "已成功将'{0}'\t复制到'{1}'",
      "Copiado exitosamente '{0}' \ta '{1}'",
      "Copié avec succès '{0}' \tvers '{1}'",
      "Başarıyla kopyalandı '{0}' \t-> '{1}'"};

    public static readonly string[] FailedCopy = {
      "Failed to copy file '{0}' \tto folder '{1}'.",
      "Не удалось скопировать файл '{0}' \tв папку '{1}'.",
      "Impossibile copiare il file '{0}' \tnella cartella '{1}'.",
      "Fehler beim Kopieren der Datei '{0}' \tin den Ordner '{1}'.",
      "无法复制文件'{0}'\tto 文件夹'{1}'。",
      "No se pudo copiar el archivo '{0}' \ta la carpeta '{1}'.",
      "Impossible de copier le fichier '{0}' \tdans le dossier '{1}'.",
      "Dosya '{0}' \tklasöre '{1}' kopyalanamadı."};

    public static readonly string[] DelOldVerErr = {
      "Error deleting old versions of file '{0}'",
      "Ошибка удаления старых версий файла '{0}'.",
      "Errore durante l'eliminazione delle vecchie versioni del file '{0}'",
      "Fehler beim Löschen alter Versionen der Datei '{0}'",
      "删除旧版本的文件'{0}'时出错",
      "Error al eliminar versiones antiguas del archivo '{0}'",
      "Erreur lors de la suppression des anciennes versions du fichier '{0}'",
      "Dosya '{0}' eski sürümleri silinirken hata"};

    public static readonly string[] OldVerErr = {
      "Error moving file '{0}' to OldVersions.\n\t{1}",
      "Ошибка перемещения файла '{0}' в OldVersions.\n\t{1}",
      "Errore durante lo spostamento del file '{0}' in OldVersions.\n\t{1}",
      "Fehler beim Verschieben der Datei '{0}' nach OldVersions.\n\t{1}",
      "将文件'{0}'移动到旧版本时出错。\n\t{1}",
      "Error al mover el archivo '{0}' a OldVersions.\n\t{1}",
      "Erreur lors du déplacement du fichier '{0}' vers OldVersions.\n\t{1}",
      "Dosya '{0}' OldVersions'a taşınırken hata.\n\t{1}"};

    public static readonly string[] DelOldFileErr = {
      "Failed to delete old file '{0}'.\n\t{1}",
      "Не получилось удалить старый файл '{0}'.\n\t{1}",
      "Impossibile eliminare il vecchio file '{0}'.\n\t{1}",
      "Fehler beim Löschen der alten Datei '{0}'.\n\t{1}",
      "未能删除旧文件'{0}'。\n\t{1}",
      "No se pudo eliminar el archivo antiguo '{0}'.\n\t{1}",
      "Impossible de supprimer l'ancien fichier '{0}'.\n\t{1}",
      "Eski dosya '{0}' silinemedi.\n\t{1}"};

    public static readonly string[] ACLCopyError = {
      "Error copying file rights '{0}'.\n\t{1}",
      "Ошибка копирования прав файла '{0}'.\n\t{1}",
      "Errore durante la copia dei diritti del file '{0}'.\n\t{1}",
      "Fehler beim Kopieren der Dateirechte '{0}'.\n\t{1}",
      "复制文件权限'{0}'时出错。\n\t{1}",
      "Error al copiar los permisos del archivo '{0}'.\n\t{1}",
      "Erreur lors de la copie des droits du fichier '{0}'.\n\t{1}",
      "Dosya hakları '{0}' kopyalanırken hata.\n\t{1}"};

    public static readonly string[] TooLongPath = {
      "Path to folder is too long. On Windows, only 248 characters are allowed. '{0}'",
      "Слишком длинный путь к папке. В Windows допускается только 248 символов. '{0}'",
      "Il percorso della cartella è troppo lungo. Su Windows, sono consentiti solo 248 caratteri. '{0}'",
      "Der Pfad zum Ordner ist zu lang. Unter Windows sind nur 248 Zeichen zulässig. '{0}'",
      "文件夹路径太长。 在 Windows 上，只允许 248 个字符。 '{0}'",
      "La ruta a la carpeta es demasiado larga. En Windows, solo se permiten 248 caracteres. '{0}'",
      "Le chemin vers le dossier est trop long. Sous Windows, seulement 248 caractères sont autorisés. '{0}'",
      "Klasör yolu çok uzun. Windows'ta yalnızca 248 karakter kullanılabilir. '{0}'"};

    public static readonly string[] ClearDirErr = {
      "Failed to clean up folder {0}: {1}",
      "Не удалось почистить папку {0}: {1}",
      "Impossibile pulire la cartella {0}: {1}",
      "Fehler beim Bereinigen des Ordners {0}: {1}",
      "未能清理文件夹 {0}：{1}",
      "No se pudo limpiar la carpeta {0}: {1}",
      "Impossible de nettoyer le dossier {0}: {1}",
      "Klasör {0} temizlenemedi: {1}"};

    public static readonly string[] FileDeleteErr = {
      "Failed to delete file {0}: {1}",
      "Не удалось удалить файл {0}: {1}",
      "Impossibile eliminare il file {0}: {1}",
      "Fehler beim Löschen der Datei {0}: {1}",
      "未能删除文件 {0}：{1}",
      "No se pudo eliminar el archivo {0}: {1}",
      "Impossible de supprimer le fichier {0}: {1}",
      "Dosya {0} silinemedi: {1}"};

    public static readonly string[] DirDeleteErr = {
      "Failed to delete directory {0}: {1}",
      "Не удалось удалить каталог {0}: {1}",
      "Impossibile eliminare la directory {0}: {1}",
      "Fehler beim Löschen des Verzeichnisses {0}}: {1}",
      "未能删除目录 {0}：{1}",
      "No se pudo eliminar el directorio {0}: {1}",
      "Impossible de supprimer le répertoire {0}: {1}",
      "Dizin {0} silinemedi: {1}"};

    public static readonly string[] DirDeleted = {
      "Deleted directory {0}",
      "Удален каталог {0}",
      "Directory eliminata {0}",
      "Gelöschtes Verzeichnis {0}",
      "已删除目录 {0}",
      "Directorio eliminado {0}",
      "Répertoire supprimé {0}",
      "Dizin silindi {0}"};

    public static readonly string[] DeletePlugin = {
      "Delete old plugin {0}...",
      "Удаление старого плагина {0}...",
      "Rimozione del vecchio plug-in {0}...",
      "Entfernen des alten Plugins {0}...",
      "删除旧插件 {0}...",
      "Eliminando complemento antiguo {0}...",
      "Suppression de l'ancien plug-in {0}...",
      "Eski eklenti {0} siliniyor..."};

    public static readonly string[] CreateLnkErr = {
      "Failed to create shortcut",
      "Сбой при создании ярлыка",
      "Impossibile creare il collegamento",
      "Verknüpfung konnte nicht erstellt werden",
      "创建快捷方式失败",
      "No se pudo crear el acceso directo",
      "Impossible de créer le raccourci",
      "Kısayol oluşturulamadı"};

    public static readonly string[] FilesProcessed = {
      "Files are being processed",
      "Обрабатываются файлы",
      "File in fase di elaborazione",
      "Dateien werden verarbeitet",
      "正在处理的文件",
      "Los archivos se están procesando",
      "Les fichiers sont en cours de traitement",
      "Dosyalar işleniyor"};

    public static readonly string[] ProcessedFiles = {
      "  Processed {0} files in {1}s",
      "  Обработано {0} файлов за {1}с",
      "  Elaborati {0} file in {1}s",
      "  {0} Dateien in {1}s verarbeitet",
      "  已处理 {0} 个文件（耗时 {1} 秒）",
      "  Procesados {0} archivos en {1}s",
      "  {0} fichiers traités en {1}s",
      "  {1} saniyede {0} dosya işlendi"};
  }
}